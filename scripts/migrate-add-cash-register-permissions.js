/**
 * Migración: agrega los permisos de Caja (LIST/OPEN/CLOSE_CASH_REGISTER) a
 * los roles administradores existentes — creados antes de que la función de
 * Caja existiera, así que no la tienen todavía.
 *
 ...
 *   node scripts/migrate-add-cash-register-permissions.js --dry-run
 *
 * Ejecución real:
 *   node scripts/migrate-add-cash-register-permissions.js
 *
...
// Los tres permisos de Caja que ya usa el código (ver ability.ts). No se
// incluye ALL_CASH_REGISTER a propósito: ningún otro módulo usa un
// permiso "ALL_*" en el rol Administrador por defecto — el patrón de este
// proyecto es la lista granular, así que se mantiene consistente.
const CASH_REGISTER_PERMISSIONS = [
  "LIST_CASH_REGISTER",
  "OPEN_CASH_REGISTER",
  "CLOSE_CASH_REGISTER",
];
...
async function migrate() {
  ...
  try {
    ...
    // no "permissions" — el script viejo (migrate-split-permissions.js)
    // usaba el nombre incorrecto y probablemente no migró nada nunca.
    ...
    for (const role of adminRoles) {
      ...
      const missing = CASH_REGISTER_PERMISSIONS.filter((p) => !currentPerms.includes(p));
