/**
 * Migración: agrega los permisos de Comisiones (COMMISSION_REPORT,
 * PAY_COMMISSION) a los roles administradores existentes — creados antes de
 * que la función de Comisiones existiera, así que no la tienen todavía.
 *
 ...
 *   node scripts/migrate-add-commission-permissions.js --dry-run
 *
 * Ejecución real:
 *   node scripts/migrate-add-commission-permissions.js
 *
...
// Los dos permisos de Comisiones que ya usa el código (ver ability.ts). No
// se incluye ALL_COMMISSION a propósito: ningún otro módulo usa un permiso
// "ALL_*" en el rol Administrador por defecto — el patrón de este proyecto
// es la lista granular, así que se mantiene consistente.
const COMMISSION_PERMISSIONS = ["COMMISSION_REPORT", "PAY_COMMISSION"];
...
async function migrate() {
  ...
  try {
    ...
    for (const role of adminRoles) {
      ...
      const missing = COMMISSION_PERMISSIONS.filter((p) => !currentPerms.includes(p));
