/**
 * Migración: agrega el permiso SELL_BELOW_MIN_PRICE a los roles
 * administradores existentes — creados antes de que el precio de venta
 * mínimo existiera, así que no lo tienen todavía. Sin este permiso, nadie
 * puede vender por debajo del mínimo configurado en un producto (ni el
 * precio ingresado ni el precio final tras descuento).
 *
 ...
 *   node scripts/migrate-add-sell-below-min-price-permission.js --dry-run
 *
 * Ejecución real:
 *   node scripts/migrate-add-sell-below-min-price-permission.js
 *
...
const MIN_PRICE_PERMISSIONS = ["SELL_BELOW_MIN_PRICE"];
...
async function migrate() {
  ...
  try {
    ...
    for (const role of adminRoles) {
      ...
      const missing = MIN_PRICE_PERMISSIONS.filter((p) => !currentPerms.includes(p));
