...
async function run() {
    ...
    const products = await product_model_1.Product.find({ company: companyObjId }).lean();
    console.log(`\n🏢  Empresa: ${companyId}`);
    ...
    for (const product of products) {
        ...
        if (product.stock_type === stockType_enum_1.stockType.INDIVIDUAL) {
            ...
            const approvedInventories = inventories.filter((inv) => inv.status !== productInventoryStatus_enum_1.productInventoryStatus.BORRADOR);
            const draftInventories = inventories.filter((inv) => inv.status === productInventoryStatus_enum_1.productInventoryStatus.BORRADOR);
            expectedStock = approvedInventories.reduce((sum, inv) => sum + inv.available + inv.reserved, 0);
            ...
            for (const inv of approvedInventories) {
                ...
                if (inv.status === productInventoryStatus_enum_1.productInventoryStatus.SIN_STOCK && hasActiveStock) {
                    issues.push({
                        type: "STATUS_MISMATCH",
                        detail: `ProductInventory ${inv._id}: status=SIN_STOCK pero available=${inv.available} reserved=${inv.reserved}`,
                    });
                }
                if (inv.status === productInventoryStatus_enum_1.productInventoryStatus.DISPONIBLE && !hasActiveStock && inv.sold === 0 && inv.transferred === 0) {
                    issues.push({
                        type: "STATUS_MISMATCH",
            ...
            // ── SERIALIZADO ──────────────────────────────────────────────────────────
            const activeSerials = await product_serial_model_1.ProductSerial.countDocuments({
                company: companyObjId,
                product: product._id,
                status: { $in: [productSerialStatus_enum_1.productSerialStatus.DISPONIBLE, productSerialStatus_enum_1.productSerialStatus.RESERVADO] },
            });
            const draftSerials = await product_serial_model_1.ProductSerial.countDocuments({
                company: companyObjId,
                product: product._id,
                status: productSerialStatus_enum_1.productSerialStatus.BORRADOR,
            });
            const soldSerials = await product_serial_model_1.ProductSerial.countDocuments({
                company: companyObjId,
                product: product._id,
            ...
            const totalSerials = await product_serial_model_1.ProductSerial.countDocuments({
                company: companyObjId,
                product: product._id,
            });
    ...
    const withRealIssues = reports.filter((r) => r.issues.some((i) => realIssueTypes.includes(i.type)));
    const withInfoOnly = reports.filter((r) => !r.issues.some((i) => realIssueTypes.includes(i.type)) &&
