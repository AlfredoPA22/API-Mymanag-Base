...
exports.assertPlanLimit = void 0;
...
const assertPlanLimit = (plan, resourceLabelPlural, currentCount, limit, opts) => {
    if (!limit || limit === Infinity || currentCount < limit)
        return;
    const planLabel = companyPlan_enum_1.PLAN_LABELS[plan] ?? plan;
    ...
    if (currentCount > limit) {
        ...
        throw new Error(`Tu plan actual (${planLabel}) permite hasta ${limit} ${resourceLabelPlural}${period}, pero ya tenés ${currentCount} (probablemente por un cambio de plan). ${hint}`);
    }
    throw new Error(`Tu plan actual (${planLabel}) solo permite hasta ${limit} ${resourceLabelPlural}${period}.`);
};
